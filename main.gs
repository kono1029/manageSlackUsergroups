...
const CELL_WORKSPACE_ID = SHEET_CONFIG.getRange(6, 2); // ワークスペースID(5行目にワークスペース名を入力してvlookupでワークスペースIDを検索する)
const CELL_LAST_TIME = SHEET_CONFIG.getRange(7, 2); // 最終実行時刻
const CELL_RESULT = SHEET_CONFIG.getRange(8, 2); // 処理結果
const CELL_PROCESSING_SEC = SHEET_CONFIG.getRange(9, 2); // 処理時間
